...
The vendored fusion-harness has an internal `Role` type (`ARCHITECT`/`BUILDER`/`FUSION`/`VALIDATOR`) used only inside `extensions/fusion-harness/`. It is not this glossary term; it names fusion stack slots, not chain/team seats.
