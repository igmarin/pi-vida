...
- `graphify-out/` is gitignored. Keep `graphify-out/memory/` absent: graphify's `detect()` hard-includes it and bypasses all ignore rules (graphifyy ≤0.9.63), so saved query notes re-enter the corpus as noise. Don't run `graphify save-result` with the default `--memory-dir` here.
